...
router.post("/matricular", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const resultsCheck = yield db_1.default.query("SELECT COUNT(*) as count FROM alumno_curso WHERE rut_alumno = $1", [rut_alumno]);
        const yaTieneMatricula = parseInt(resultsCheck.rows[0].count) > 0;
        // 2️⃣ Insertar inscripción
        const resultInsert = yield db_1.default.query(`INSERT INTO alumno_curso (rut_alumno, id_curso, fecha_inscripcion, estado)
       VALUES ($1, $2, $3, 'activo') RETURNING id`, [rut_alumno, id_curso, fecha]);
        const id_alumno_curso = resultInsert.rows[0].id;
        // 3️⃣ Obtener información del curso
        const resultsCurso = yield db_1.default.query("SELECT nombre_curso, costo_curso, duracion_curso FROM curso WHERE id_curso = $1", [id_curso]);
        ...
        const resultsMat = yield db_1.default.query("SELECT monto FROM matricula LIMIT 1");
        ...
        // 5️⃣ Insertar deuda
        const resultDeuda = yield db_1.default.query(`INSERT INTO deuda 
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id_deuda`, [
        ...
        const resultsEmail = yield db_1.default.query(`SELECT u.correo, a.nombre_completo, u.id_usuario
       FROM alumno a
       JOIN usuario u ON a.id_usuario = u.id_usuario
       WHERE a.rut = $1`, [rut_alumno]);
        if (resultsEmail.rows.length > 0) {
            ...
            yield db_1.default.query("INSERT INTO password_reset_tokens (id_usuario, token, expiracion) VALUES ($1, $2, $3)", [idUsuario, token, expiracion]);
            const enlace = `http://localhost:5173/enlace-password?token=${token}`;
            // Enviar correo
            yield (0, correo_1.enviarCorreo)(alumnoCorreo, `Matrícula y Detalles del Curso ${nombreCurso} - AulaConnect Educa`, `
...
// Retirar alumno
router.put("/retirar/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { estadoActual } = req.body;
        const nuevoEstado = estadoActual === "activo" ? "retirado" : "activo";
        yield db_1.default.query("UPDATE alumno_curso SET estado = $1 WHERE id = $2", [
            nuevoEstado,
            id,
        ]);
...
router.get("/cursos/:rut_alumno", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      ORDER BY ac.fecha_inscripcion DESC
    `;
        const result = yield db_1.default.query(sql, [rut_alumno]);

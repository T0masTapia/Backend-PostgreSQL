...
cron.schedule("50 10 * * *", async () => {
  ...
  try {
    ...
    const { rows: deudas } = await db.query(query);
    ...
    for (const deuda of deudas) {
      ...
      await enviarCorreo(
        deuda.correo,
        `🔔 Recordatorio de Pago - Curso ${deuda.nombre_curso}`,
        `
    ...
    <div style="max-width:600px; margin:0 auto; background:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
      ...
      <div style="padding:20px; color:#333;">
        ...
        <p>
          ...
          <span style="font-size:16px; color:#2980b9;"><strong>${
            deuda.nombre_curso
          }</strong></span>
        </p>
        ...
        <table style="width:100%; margin:20px 0; border-collapse:collapse;">
          <tr>
            ...
            <td style="padding:8px; border:1px solid #ddd; color:#c0392b;">$${Number(
              deuda.monto
            ).toLocaleString()}</td>
          </tr>
          <tr>

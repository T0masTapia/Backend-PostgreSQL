...
router.get("/toda/curso/:idCurso", async (req, res) => {
...
router.get("/:idCurso/:fecha", async (req, res) => {
  ...
  try {
    const sql = `
      ...
      WHERE a.id_curso = $2
    `;
    const result = await db.query(sql, [fecha, idCurso]);
    res.json({ success: true, asistencia: result.rows });
  } catch (err: any) {
    console.error(err);
    res.status(500).json({ success: false, error: "Error al obtener asistencia" });
...
router.post("/guardar-asistencia", async (req, res) => {
...
router.get("/descargar/:idCurso/:fecha", async (req, res) => {
  ...
  try {
    // Nombre del curso
    const cursoResult = await db.query(`SELECT nombre_curso FROM curso WHERE id_curso = $1`, [idCurso]);
    ...
    const separarApellidosYNombres = (nombreCompleto: string) => {
    ...
    const colorAsistencia = (estado: string) => {
      switch (estado) {
    ...
    (result.rows as any[]).forEach((row, index) => {
